<x-lareon::admin-index-layout>
    ...
    @section('index')
        <x-lareon::box>
            <x-lareon::table :headers="['id'=>'#','featured_image','name'=>__('name'),'email'=>__('email') ,'phone'=>__('phone') ,]">
                @foreach($users as $key=>$user)
                    <tr>
                        <td class="p-3">{{$users->firstItem() + $key}}</td>
                        <td>
                            <img src="{{$user->featured_image}}" alt="{{$user->name}}" class="rounded-full w-6 h-6" fetchpriority="low" decoding="async" loading="lazy">
                        </td>
                        <td>{{$user->name}}</td>
                        <td>{{$user->email}}</td>
                        <td>{{$user->phone}}</td>
