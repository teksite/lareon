...
<div class="mb-3" xmlns:x-lareon="http://www.w3.org/1999/html">
    ...
    @endif
    <x-lareon::input.text type="text" :name="$name" id="input-{{$random}}" placeholder="{{__('choose a photo')}}" :value="$value ?? ''" :required="$required"/>
    <x-lareon::button.solid color="violet" class="w-full block" type="button" role="button" :name="$name" id="btn-{{$random}}" placeholder="{{__('choose a photo')}}" :value="$value ?? ''" :required="$required">
        {{__('choose')}}
