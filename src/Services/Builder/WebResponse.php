...
class WebResponse
{
    ...
    {
        if ($result->success) {
            $this->message($success_message ?? __('successfully done'))->result(ResponseType::SUCCESS)->data($result->result)->route($success_route);
        } else {
            $this->message($failed_message ?? __('unfortunately failed'))->result(ResponseType::FAILED)->data($result->result)->route($failed_route);
        }
        return $this;
